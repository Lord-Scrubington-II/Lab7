...
const router = new Router(function () {
   ...
   */
   document.querySelector('section[class*="section--recipe-cards"]').classList.add("shown");
   document.querySelector('section[class*="section--recipe-expand"]').classList.remove("shown");
});
...
function bindEscKey() {
  ...
  document.addEventListener("keydown", (event) => {
    if(event.key === "Escape") {
      ...
      //history.pushState({name: window.location.hash.substring(1)}, "", window.location.pathname);
    }
  });
}
...
function bindPopstate() {
  ...
  window.addEventListener('popstate', (event) => {
    console.log(event);
    let page = null;
    if(event.state === null || event.state.name === null) {
      page = "home";
    } else {
